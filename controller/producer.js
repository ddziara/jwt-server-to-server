...
async function produce(req, res) {
  ...
  try {
    ...
    console.log(
      `[${config.serverName}] sending request to consumer: ${partnerServerHostAndPort}`
    );

    const payload = {
      origin: `this message is in the body HTTP request and comes from ${config.serverName}`,
    };
  ...
  console.log(`[${config.serverName}] data exchange successfully finished`);
  res.sendStatus(204);
}
